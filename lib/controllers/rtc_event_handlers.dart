...
Future<RtcEngineEventHandler> rtcEngineEventHandler(
  ...
  return RtcEngineEventHandler(onRejoinChannelSuccess: (connection, elapsed) {
    ...
    agoraEventHandlers.onClientRoleChangeFailed?.call(connection, reason, currentRole);
  }, onConnectionStateChanged: (connection, state, reason) {
    agoraEventHandlers.onConnectionStateChanged?.call(connection, state, reason);
  }, onNetworkQuality: (connection, remoteUid, txQuality, rxQuality) {
    agoraEventHandlers.onNetworkQuality?.call(connection, remoteUid, txQuality, rxQuality);
    ...
    agoraEventHandlers.onProxyConnected?.call(channel, uid, proxyType, localProxyIp, elapsed);
  }, onRequestToken: (connection) {
    agoraEventHandlers.onRequestToken?.call(connection);
  }, onAudioVolumeIndication: (connection, speakers, speakerNumber, totalVolume) {
    agoraEventHandlers.onAudioVolumeIndication?.call(connection, speakers, speakerNumber, totalVolume);
  }, onFirstLocalAudioFramePublished: (connection, elapsed) {
    agoraEventHandlers.onFirstLocalAudioFramePublished?.call(connection, elapsed);
    ...
    agoraEventHandlers.onFirstRemoteAudioDecoded?.call(connection, uid, elapsed);
  }, onFirstRemoteAudioFrame: (connection, userId, elapsed) {
    agoraEventHandlers.onFirstRemoteAudioFrame?.call(connection, userId, elapsed);
  }, onFirstRemoteVideoFrame: (connection, remoteUid, width, height, elapsed) {
    agoraEventHandlers.onFirstRemoteVideoFrame?.call(connection, remoteUid, width, height, elapsed);
  }, onFirstRemoteVideoDecoded: (connection, remoteUid, width, height, elapsed) {
    agoraEventHandlers.onFirstRemoteVideoDecoded?.call(connection, remoteUid, width, height, elapsed);
  }, onUserMuteVideo: (connection, remoteUid, muted) {
    log("onUserMuteVideo- muted: $muted, uid: $remoteUid", name: tag, error: Level.info.value);
    ...
    log("onUserMuteAudio- muted: $muted, uid: $remoteUid", name: tag, error: Level.info.value);
    if (remoteUid != sessionController.value.localUid) {
    ...
    agoraEventHandlers.onVideoSizeChanged?.call(connection, sourceType, uid, width, height, rotation);
  }, onLocalPublishFallbackToAudioOnly: (isFallbackOrRecover) {
    agoraEventHandlers.onLocalPublishFallbackToAudioOnly?.call(isFallbackOrRecover);
  }, onRemoteSubscribeFallbackToAudioOnly: (uid, isFallbackOrRecover) {
    agoraEventHandlers.onRemoteSubscribeFallbackToAudioOnly?.call(uid, isFallbackOrRecover);
    ...
    agoraEventHandlers.onStreamMessage?.call(connection, remoteUid, streamId, data, length, sentTs);
  }, onStreamMessageError: (connection, remoteUid, streamId, code, missed, cached) {
    agoraEventHandlers.onStreamMessageError?.call(connection, remoteUid, streamId, code, missed, cached);
    ...
    agoraEventHandlers.onAudioPublishStateChanged?.call(channel, oldState, newState, elapseSinceLastState);
  }, onVideoPublishStateChanged: (source, channel, oldState, newState, elapseSinceLastState) {
    agoraEventHandlers.onVideoPublishStateChanged?.call(source, channel, oldState, newState, elapseSinceLastState);
  }, onAudioSubscribeStateChanged: (channel, uid, oldState, newState, elapseSinceLastState) {
    agoraEventHandlers.onAudioSubscribeStateChanged?.call(channel, uid, oldState, newState, elapseSinceLastState);
  }, onVideoSubscribeStateChanged: (channel, uid, oldState, newState, elapseSinceLastState) {
    agoraEventHandlers.onVideoSubscribeStateChanged?.call(channel, uid, oldState, newState, elapseSinceLastState);
  }, onUploadLogResult: (connection, requestId, success, reason) {
    agoraEventHandlers.onUploadLogResult?.call(connection, requestId, success, reason);
  }, onError: (err, msg) {
    ...
    sessionController.value = sessionController.value.copyWith(localUid: connection.localUid);

    await rtmMethods(agoraRtmChannelEventHandler: agoraRtmChannelEventHandler, sessionController: sessionController);
    ...
    final String info = "Remote video state changed for $remoteUid, state: $state and reason: $reason";
    log(info, name: tag, level: Level.info.value);
    if (remoteUid != sessionController.value.localUid) {
    ...
    agoraEventHandlers.onRemoteVideoStateChanged?.call(connection, remoteUid, state, reason, elapsed);
  }, onRemoteAudioStateChanged: (connection, remoteUid, state, reason, elapsed) {
    final String info = "Remote audio state changed for $remoteUid, state: $state and reason: $reason";
    log(info, name: tag, level: Level.info.value);
    ...
    agoraEventHandlers.onRemoteAudioStateChanged?.call(connection, remoteUid, state, reason, elapsed);
    ...
    final String info = "Local video state changed, source: $source, state: $state and error: $error";
    log(info, name: tag, level: Level.info.value);

    if (source == VideoSourceType.videoSourceScreenPrimary &&
        state == LocalVideoStreamState.localVideoStreamStateCapturing) {
      sessionController.value = sessionController.value.copyWith(isScreenShared: true);
    } else {
      sessionController.value = sessionController.value.copyWith(isScreenShared: false);
    }
    ...
    agoraEventHandlers.onAudioDeviceStateChanged?.call(deviceId, deviceType, deviceState);
  }, onAudioDeviceVolumeChanged: (deviceType, volume, muted) {
    agoraEventHandlers.onAudioDeviceVolumeChanged?.call(deviceType, volume, muted);
  }, onAudioQuality: (connection, remoteUid, quality, delay, lost) {
    agoraEventHandlers.onAudioQuality?.call(connection, remoteUid, quality, delay, lost);
  }, onCameraReady: () {
  ...
  }, onRemoteAudioTransportStats: (connection, remoteUid, delay, lost, rxKBitRate) {
    agoraEventHandlers.onRemoteAudioTransportStats?.call(connection, remoteUid, delay, lost, rxKBitRate);
  }, onRemoteVideoTransportStats: (connection, remoteUid, delay, lost, rxKBitRate) {
    agoraEventHandlers.onRemoteVideoTransportStats?.call(connection, remoteUid, delay, lost, rxKBitRate);
    ...
    agoraEventHandlers.onSnapshotTaken?.call(connection, uid, filePath, width, height, errCode);
  }, onUserAccountUpdated: (connection, remoteUid, userAccount) {
    agoraEventHandlers.onUserAccountUpdated?.call(connection, remoteUid, userAccount);
  }, onUserEnableLocalVideo: (connection, remoteUid, enabled) {
    agoraEventHandlers.onUserEnableLocalVideo?.call(connection, remoteUid, enabled);
    ...
    agoraEventHandlers.onVideoDeviceStateChanged?.call(deviceId, deviceType, deviceState);
  }, onVideoStopped: () {
    agoraEventHandlers.onVideoStopped?.call();
  }, onWlAccMessage: (connection, reason, action, wlAccMsg) {
    agoraEventHandlers.onWlAccMessage?.call(connection, reason, action, wlAccMsg);
  }, onWlAccStats: (connection, currentStats, averageStats) {
    agoraEventHandlers.onWlAccStats?.call(connection, currentStats, averageStats);
  });
}
